...
export default function NavDropdown({ label, items, icon, className }: NavDropdownProps) {
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        align="start"
        className="min-w-[220px] bg-popover/95 backdrop-blur-md border-primary/20 shadow-xl z-[200]"
      >
        {items.map((item, index) => (
          <DropdownMenuItem key={index} asChild>
            <Link
              to={item.href}
              className="flex items-center gap-3 px-3 py-2.5 cursor-pointer hover:bg-primary/5 hover:text-primary focus:bg-primary/5 focus:text-primary transition-colors"
            >
              {item.icon && <span className="text-primary">{item.icon}</span>}
              <div className="flex flex-col">
                <span className="font-medium">{item.label}</span>
                {item.description && (
                  <span className="text-xs text-muted-foreground dark:text-foreground/70">{item.description}</span>
                )}
              </div>
            </Link>
          </DropdownMenuItem>
